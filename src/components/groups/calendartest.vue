      ...
      >
        <template #day="{ timestamp }">
          <template>
            <template v-for="(event, index) in getEvents(timestamp.date)">
              <q-badge
                :key="index"
                style=" cursor: pointer; height: 14px; max-height: 14px"
        ...
        <template #intervals-header="days">
          <div class="fit flex justify-center items-end">
            <span class="q-calendar-daily__interval-text">{{
              showOffset(days)
            }}</span>
          </div>
        </template>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      titlebarHeight: "calendar/titlebarHeight",
  ...
  methods: {
            ...
            if (events.length > 0) {
              ...
              for (let j = 0; j < events.length; ++j) {
                ...
                if (evt.time !== undefined) {
                  ...
                  if (
                    isBetweenDates(startTime, startTime2, endTime2) ||
                    isBetweenDates(endTime, startTime2, endTime2)
                  ) {
                    evt.side = "left";
                    event.side = "right";
                    events.push(event);
    ...
    checkDateTimeStart(/* val */) {
      ...
      if (this.eventForm.dateTimeStart && this.eventForm.dateTimeEnd) {
        const timestampStart = parseTimestamp(this.eventForm.dateTimeStart);
        const timestampEnd = parseTimestamp(this.eventForm.dateTimeEnd);
    ...
    getEndTime(event) {
      let endTime = parseTimestamp(event.date + " " + event.time);
      endTime = addToDate(endTime, { minute: event.duration });
    ...
    editEvent(event) {
      ...
      let timestamp;
      if (event.time) {
        timestamp = parseTimestamp(event.date + " " + event.time);
        const endTime = addToDate(timestamp, {
          minute: event.duration
        });
        this.eventForm.dateTimeEnd = getDateTime(endTime);
      } else {
    ...
    showOffset(days) {
      ...
      const val = padTime(
        new Date(this.getTimestampString(days[0])).getTimezoneOffset() / 60
      );
      if (isNaN(val)) return "";
      return "GMT-" + val;
    },
    ...
    getTimestampString(day) {
      return getDate(day) + " " + getTime(day);
    },
    updateFormatters() {
      try {
